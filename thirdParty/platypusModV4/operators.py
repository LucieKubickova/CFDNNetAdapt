...
class expDecBitFlip(Mutation):

    def __init__(self, startProb, halflife):
        """Bit Flip Mutation with exponentially decreasing mutation rate for Binary Strings.
        
        Parameters
        ----------
        startProb : int or float
            The starting probability of flipping an individual bit.

        halflife : number of evaluations after which the probability decreases to half of its value
        """

        super(expDecBitFlip, self).__init__()
    ...
    def mutate(self, parent):
        ...
        probability = startProb*math.exp(-math.log(2)*nfe/halflife)
        threshold = 1.0/sum([t.nbits for t in problem.types if isinstance(t, Binary)])
